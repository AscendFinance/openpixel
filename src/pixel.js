class Pixel {
  ...
  getAttribute() {
    return {
      ...
      uid:          () => Cookie.get('uid') || LocalStorage.get('uid'), // user Id
      ...
      utm_source:   key => Cookie.getUtm(key) || LocalStorage.getUtm(key), // get the utm source
      utm_medium:   key => Cookie.getUtm(key) || LocalStorage.getUtm(key), // get the utm medium
      utm_term:     key => Cookie.getUtm(key) || LocalStorage.getUtm(key), // get the utm term
      utm_content:  key => Cookie.getUtm(key) || LocalStorage.getUtm(key), // get the utm content
      utm_campaign: key => Cookie.getUtm(key) || LocalStorage.getUtm(key), // get the utm campaign
      ...Config.params
    }
  }
